function [ tranQueue, arrivalQueue, bufferQueue, last_end_slot_ind] = nodeTranPerFrame_Fixed(cur_ind_frame, cur_shadow, cur_EH_collect, last_end_slot_ind, Allocate, Node, MAC, Channel, Constraints, tranQueue, arrivalQueue, bufferQueue, rand_seed)
...
% %     tranQueue=[tranQueue; deletePackets];
    ...
    if sum(Allocate.slot)<=0
        ...
        if residue_energy > Node.battery_capacity %电池容量有上限
            residue_energy = Node.battery_capacity;
        end
