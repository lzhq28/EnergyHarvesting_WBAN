    ...
    for ind_frame = 1:N_Frame %对超帧进行遍历
        ...
        for ind_node = 1:par.Nodes.Num %对各个节点进行遍历
           ...
           %% 配置参数
            ...
            %% 分析数据包到达队列,这里只统计正常包            
            % 正常数据包
            cur_arrival_num_packets = ceil(Node(ind_node).Nor_SrcRate*(end_slot_ind + par.MAC.N_Slot - last_end_slot_ind)*par.MAC.T_Slot/Node(ind_node).packet_length);
            ...
            last_time_frame = (par.MAC.N_Slot - last_end_slot_ind)*par.MAC.T_Slot; %上一超帧所剩余的时间
            ...
            %% 分析数据包传输队列
            tran_times = floor((end_slot_ind + par.MAC.N_Slot - last_end_slot_ind)*par.MAC.T_Slot.*Allocate(ind_node).rate./Node(ind_node).packet_length); %分配的资源可以传输的次数
