function []=compareDecide(PNoise,deltaPL,powerFactor)

%% 
%powerFactor 用来调节功率的因子，
% 
% load(strcat('channel_Pnoise',num2str(PNoise),'_deltaPL',num2str(deltaPL),'.mat'))
% load(strcat('shadow_Pnoise',num2str(PNoise),'_deltaPL',num2str(deltaPL),'.mat'))
% load(strcat('numPacket_Pnoise',num2str(PNoise),'_deltaPL',num2str(deltaPL),'.mat'))
% load(strcat('optimalValue_Pnoise',num2str(PNoise),'_deltaPL',num2str(deltaPL),'.mat'))
PLRInfo='PLRN0.3E0.275';
...
%         scheRate(1:N_Node,1,m)=posRate{1}(1:2:end,1); %这里设置为最大值
%         scheRate(1:N_Node,2,m)=posRate{1}(2:2:end,1); %这里设置为最大值         
%      elseif m<=(probPosture(1)+probPosture(2))*N_ch
%          scheRate(1:N_Node,1,m)=posRate{2}(1:2:end,1); %这里设置为最大值
%          scheRate(1:N_Node,2,m)=posRate{2}(2:2:end,1); %这里设置为最大值   
%      else
%          scheRate(1:N_Node,1,m)=posRate{3}(1:2:end,1); %这里设置为最大值
%          scheRate(1:N_Node,2,m)=posRate{3}(2:2:end,1); %这里设置为最大值   
%      end;
             %根据信道信息分配资源，这里一般只对功率进行配置
    ...
    if decideFlag==1
     ...
     if sum(sum(scheTime(:,:,m)))>T_Frame
          disp(['warnning in compareDecide: sum(t)=',num2str( sum(sum(scheTime(:,:,m)))),'>T_Frame'])
     end;
     %powerFactor=0.7;
     schePower(:,1,m)=powerFactor*repmat(power(10,P_tx_max_dBm/10),N_Node,1);%初始化为1mW，系数为0.15是性能还不错
     schePower(:,2,m)=powerFactor*repmat(power(10,P_tx_max_dBm/10),N_Node,1);%初始化为1mW
    ...
    curNumTranNormal=round(scheTime(:,1,m).*scheRate(:,1,m)./Normal_L_packet');%分配的时隙可以发送包的次数
    curNumTranEmergency=round(scheTime(:,2,m).*scheRate(:,2,m)./Emergency_L_packet');%分配的时隙可以发送包的次数
...
% save(strcat('all1_Pnoise',num2str(PNoise),'_deltaPL',num2str(deltaPL),'.mat'))%保存所有的数据
% save(strcat('finalResult1_Pnoise',num2str(PNoise),'_deltaPL',num2str(deltaPL),'.mat'),'result*','packetDelayNormal','packetDelayEmergency');

DecideInfo=strcat('Decide_PowerFactor',num2str(powerFactor))
save(strcat(DecideInfo,'_all2_Pnoise',num2str(PNoise),'_deltaPL',num2str(deltaPL),'.mat'));%保存所有的数据
save(strcat(DecideInfo,'_finalResult2_Pnoise',num2str(PNoise),'_deltaPL',num2str(deltaPL),'.mat'),'result*','packetDelayNormal','packetDelayEmergency');